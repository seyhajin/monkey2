...
function Main()
	...
	desc.window_title = CStr("sokol-non_interleaved")
	desc.width = 800
	desc.height = 600
...
function init:void()
	...
	'/* note how the vertex components are pulled 
	'	from different buffer bind slots
	'*/
	
	'// positions come from vertex buffer slot 0
	pp.layout.attrs[0].format = SG_VERTEXFORMAT_FLOAT3 ; pp.layout.attrs[0].buffer_index = 0
	'// colors come from vertex buffer slot 1
	pp.layout.attrs[1].format = SG_VERTEXFORMAT_FLOAT4 ; pp.layout.attrs[1].buffer_index = 1
	...
	state.bind.vertex_buffers[1] = sg_make_buffer(varptr vb)
	state.bind.vertex_buffer_offsets[0] = 0
	state.bind.vertex_buffer_offsets[1] = 24 * 3 * libc.sizeof<float>()
